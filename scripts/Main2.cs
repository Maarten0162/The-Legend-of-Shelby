...
public partial class Main2 : Node2D
{
			...
			{ // switch voor north
				...
				switch (GlobalVar.Instance.exit)
				{
					case "Room2south":
						...
						{
							GetNode<Door>("Y-Sort/Area2D").QueueFree();
						}
	...
	{	       if(GlobalVar.Instance.HasSword){
            player.weaponCollision = GetNode<CollisionShape2D>("weapon/CollisionShape2D");
            player.weaponSprite = GetNode<Sprite2D>("weapon/Sprite2D");
            player.weaponBody = GetNode<Area2D>("weapon/");



            player.weaponSprite.Hide();
            player.weaponCollision.Disabled = true;
